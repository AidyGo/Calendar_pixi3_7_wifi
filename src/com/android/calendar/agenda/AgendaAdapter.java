...
public class AgendaAdapter extends ResourceCursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (holder == null) {
            ...
            //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13end
        }
...
//        holder.colorChip.setLayoutParams(params);
        long startDay = cursor.getLong(AgendaWindowAdapter.INDEX_START_DAY);
        long endDay = cursor.getLong(AgendaWindowAdapter.INDEX_END_DAY);
        
        if (allDay) {
            holder.img.setBackgroundResource(R.drawable.event_allday);
        } else if(endDay > startDay){   //stride day event
            holder.img.setBackgroundResource(R.drawable.event_strideday);
        } else {
        ...
        if (Utils.IS_FACEBOOK_DEVICE
                && !TextUtils.isEmpty(cursor.getString(AgendaWindowAdapter.INDEX_FBFRIEND_ID))) {
        	// highlight the birthday with the color returned by Utils.getFBBithdayColor().
        	//holder.colorChip.setColor(Utils.getFBBithdayColor());//deleted by yubin
        	 holder.img.setBackgroundResource(R.drawable.event_allday);//added by yubin
        }
