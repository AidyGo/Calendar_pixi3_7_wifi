...
//import android.text.format.DateUtils;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
//            holder.dayView.setText(dayViewText);
            ...
            Time d = new Time();
            d.setJulianDay(row.mDay);
            d.normalize(true);
            Time currentTime = new Time();
            currentTime.set(System.currentTimeMillis());
            boolean isSameY = d.year == currentTime.year;
            boolean isSameM = d.month == currentTime.month;
            boolean isSameD = d.monthDay == currentTime.monthDay;
            int datebackground;
			if (isSameY && isSameM && isSameD) {
				datebackground = mContext.getResources().getColor(R.color.agenda_today_background);
			} else {
				datebackground = mContext.getResources().getColor(R.color.agenda_day_background);
			}
            String WEEKDAY_FORMAT = "%a";
            String dateViewText = String.format("%d/%d\t%s", d.month+1,d.monthDay,d.format(WEEKDAY_FORMAT));
            holder.dateView.setBackgroundColor(datebackground);
