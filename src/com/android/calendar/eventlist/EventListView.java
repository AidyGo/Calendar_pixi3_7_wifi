...
import com.android.calendar.eventlist.EventAdapter.ViewHolder;
import com.android.calendar.eventlist.EventWindowAdapter.DayAdapterInfo;
import com.android.calendar.eventlist.EventWindowAdapter.EventInfo;
...
public class EventListView extends ListView implements OnItemClickListener {

    private static final String TAG = "EventListView";
    private static final boolean DEBUG = false;
    private static final int EVENT_UPDATE_TIME = 300000;  // 5 minutes

    public EventWindowAdapter mWindowAdapter;
    ...
    private void initView(Context context) {
        ...
        if (context instanceof com.mediatek.calendar.selectevent.AgendaChoiceActivity) {
            ...
            mWindowAdapter = new EventWindowAdapter(context, this, false);
        } else {
            mWindowAdapter = new EventWindowAdapter(context, this, Utils.getConfigBool(context,
    ...
    public void onItemClick(AdapterView<?> a, View v, int position, long id) {
        if (id != -1) {
            ...
            if (event != null && (oldInstanceId != mWindowAdapter.getSelectedInstanceId() ||
                ...
                if (holder instanceof EventAdapter.ViewHolder) {
                    holderStartTime = ((EventAdapter.ViewHolder) holder).startTimeMilli;
                } else {
    ...
    public long getFirstVisibleTime(Object e) {
        EventInfo event = (EventInfo)e;
        if (e == null) {
    ...
    private void shiftPosition(int offset) {
        ...
        if (firstVisibleItem != null) {
            ...
            if (DEBUG) {
                if (firstVisibleItem.getTag() instanceof EventAdapter.ViewHolder) {
                    ViewHolder viewHolder = (EventAdapter.ViewHolder) firstVisibleItem.getTag();
                    Log.v(TAG, "Shifting from " + position + " by " + offset + ". Title "
                            + viewHolder.title.getText());
                } else if (firstVisibleItem.getTag() instanceof EventByDayAdapter.ViewHolder) {
                    EventByDayAdapter.ViewHolder viewHolder =
                            (EventByDayAdapter.ViewHolder) firstVisibleItem.getTag();
