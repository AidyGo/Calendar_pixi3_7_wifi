...
import com.android.calendar.eventlist.EventWindowAdapter.DayAdapterInfo;
...
public class EventByDayAdapter extends BaseAdapter {
    private static final String TAG = EventByDayAdapter.class.getSimpleName();
    ...
    public EventByDayAdapter(Context context) {
        mContext = context;
        mEventAdapter = new EventAdapter(context, R.layout.agenda_item);
    ...
    public Object getItem(int position) {
        if (mRowInfo != null) {
            ...
            if (row.mType == TYPE_DAY) {
                ...
                return mEventAdapter.getItem(row.mPosition);
            }
        }
        return mEventAdapter.getItem(position);
    }
    ...
    public long getItemId(int position) {
        if (mRowInfo != null) {
            ...
            if (row.mType == TYPE_DAY) {
                ...
                return mEventAdapter.getItemId(row.mPosition);
            }
        }
        return mEventAdapter.getItemId(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if ((mRowInfo == null) || (position > mRowInfo.size())) {
            // If we have no row info, mEventAdapter returns the view.
            return mEventAdapter.getView(position, convertView, parent);
        }
        ...
        if (row.mType == TYPE_DAY) {
            ...
            if (EventWindowAdapter.BASICLOG) {
            ...
            View itemView = mEventAdapter.getView(row.mPosition, convertView, parent);
            EventAdapter.ViewHolder holder = ((EventAdapter.ViewHolder) itemView.getTag());
    ...
    public void changeCursor(DayAdapterInfo info) {
        calculateDays(info);
        mEventAdapter.changeCursor(info.cursor);
    }
    ...
    public void calculateDays(DayAdapterInfo dayAdapterInfo) {
        ...
        int prevStartDay = -1;
        //PR514240-yanchao.chen-001 start
        Time time = CalendarController.getInstance(mContext).getCalendarTime();
        long now = time.toMillis(false);
        if(!TextUtils.equals(time.timezone, mTimeZone)){
           mTimeZone = Utils.getTimeZone(mContext, mTZUpdater);
         }
        Time tempTime = new Time(mTimeZone);
        //PR514240-yanchao.chen-001 end
        tempTime.set(now);
        ...
        for (int position = 0; cursor.moveToNext(); position++) {
            int startDay = cursor.getInt(EventWindowAdapter.INDEX_START_DAY);
            long id = cursor.getLong(EventWindowAdapter.INDEX_EVENT_ID);
            long startTime =  cursor.getLong(EventWindowAdapter.INDEX_BEGIN);
            long endTime =  cursor.getLong(EventWindowAdapter.INDEX_END);
            long instanceId = cursor.getLong(EventWindowAdapter.INDEX_INSTANCE_ID);
            ...
            int endDay = cursor.getInt(EventWindowAdapter.INDEX_END_DAY);
            ...
            if(startDay < mTodayJulianDay && mTodayJulianDay <= endDay)
            {
