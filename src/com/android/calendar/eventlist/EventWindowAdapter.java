...
public class EventWindowAdapter extends BaseAdapter
    ...
    private static final String TAG = "EventWindowAdapter";
    ...
    private final EventListView mEventListView;
    ...
    private static int mTopDeviation; // Save the top deviation to make switch screen smoothly.
    ...
    public static void setTopDeviation(int topDeviation) {
        mTopDeviation = topDeviation;
    }
    /// @}
    ...
    public int saveTopDeviation(Time time) {
        ...
        int firstVisiblePosition = mEventListView.getFirstVisiblePosition();
        View firstChildView = mEventListView.getChildAt(0);
        ...
        if (isDayType || isTopIsHeader) {
            ...
            if (isDayType) {
                ...
                View firstDayTypeView = mEventListView.getChildAt(1);
        ...
        if (!isDayType) {
            ...
            /// M: If isTopIsHeader is true, this item is a header, so add itself height.
            if (isTopIsHeader) {
                num++;
            } else {
                /// M: This is a MEETING TYPE item, add the items height above it, the items which
                //  must happened at the same time.
                int firstItemOnSameTime = findEventPositionNearestTime(time, -1);
                int i = firstVisiblePosition - 1;
                while (i > firstItemOnSameTime && getItemViewType(i - 1) == 1) {
                    num--;
                    i--;
                }
            }
            topDeviation += num * meetingTypeItemHeight;
        }
        mTopDeviation = topDeviation;
        if (DEBUGLOG) {
            Log.e(TAG, "save topDeviation height is : " + topDeviation);
        }
        return topDeviation;
    }
    /// @}
    ...
    static class DayAdapterInfo {
        ...
        public DayAdapterInfo(Context context) {
            dayAdapter = new EventByDayAdapter(context);
        }
    ...
    public EventWindowAdapter(Context context,
            EventListView EventListView, boolean showEventOnStart) {
        mContext = context;
...
//        mEventListView.addHeaderView(mHeaderView);
    }

    // Method in Adapter
    @Override
    public int getViewTypeCount() {
        return EventByDayAdapter.TYPE_LAST;
    }
    ...
    public long getItemId(int position) {
        ...
        if (info != null) {
            ...
            if (curPos >= 0) {
                ...
                return info.cursor.getLong(EventWindowAdapter.INDEX_EVENT_ID) << 20 +
                    info.cursor.getLong(EventWindowAdapter.INDEX_BEGIN);
            }
            // Day Header
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Object yy = v.getTag();
        if (yy instanceof EventAdapter.ViewHolder) {
            EventAdapter.ViewHolder vh = (EventAdapter.ViewHolder) yy;
    ...
    private EventAdapter.ViewHolder mSelectedVH = null;
    ...
    private EventInfo buildEventInfoFromCursor(final Cursor cursor, int cursorPosition,
        ...
        event.begin = cursor.getLong(EventWindowAdapter.INDEX_BEGIN);
        event.end = cursor.getLong(EventWindowAdapter.INDEX_END);
        event.startDay = cursor.getInt(EventWindowAdapter.INDEX_START_DAY);

        event.allDay = cursor.getInt(EventWindowAdapter.INDEX_ALL_DAY) != 0;
        if (event.allDay) { // UTC
        ...
        if (!isDayHeader) {
            ...
            event.id = cursor.getLong(EventWindowAdapter.INDEX_EVENT_ID);
        }
        return event;
    }
    ...
    public void refresh(Time goToTime, long id, String searchQuery, boolean forced,
        ...
        if (!mCleanQueryInitiated || searchQuery != null) {
            ...
            // Pre-fetch more data to overcome a race condition in EventListView.shiftSelection
...
//                /// @}
//                mEventListView.addFooterView(mFooterView);
...
//            }
            ...
            if (cursorSize > 0 || mAdapterInfos.isEmpty() || data.queryType == QUERY_TYPE_CLEAN) {
                ...
                if (data.goToTime == null) { // Typical Scrolling type query
                    ...
                    if (listPositionOffset != 0) {
                        mEventListView.shiftSelection(listPositionOffset);
                    }
                    ...
                    if (newPosition >= 0) {
                        ...
                        mEventListView.setSelectionFromTop(newPosition + OFF_BY_ONE_BUG,
                                mTopDeviation);
                        Time actualTime = new Time(mTimeZone);
                        actualTime.set(goToTime);
                    ...
                    if (DEBUGLOG) {
                        Log.e(TAG, "Setting listview to " + (newPosition + OFF_BY_ONE_BUG) +
                                "   add top deviation height: " + mTopDeviation);
                    }
                }
              //FR593012-Wentao-Wan-001 begin
                if(mContext != null ){
                   Intent i = new Intent(MonthByWeekFragment.EVENTS_COUNT_CHANGE_BROADCAST_FOR_UPDATE);
                   i.putExtra("Count", mEventListView.getCount());
                ...
                if (mAdapterInfos.size() == 1 && mSelectedInstanceId != -1) {
                    ...
                    while (cursor.moveToNext()) {
                        if (mSelectedInstanceId == cursor
                                .getLong(EventWindowAdapter.INDEX_INSTANCE_ID)) {
                            found = true;
                            break;
                        }
                    };
                ...
                if (mShowEventOnStart && data.queryType == QUERY_TYPE_CLEAN) {
                    ...
                    if (mSelectedInstanceId == -1) {
                        if (cursor.moveToFirst()) {
                            ...
                            mSelectedVH = new EventAdapter.ViewHolder();
                            mSelectedVH.allDay =
                                cursor.getInt(EventWindowAdapter.INDEX_ALL_DAY) != 0;
                            tempCursor = cursor;
                        }
    ...
    static String getViewTitle(View x) {
        ...
        if (x != null) {
            Object yy = x.getTag();
            if (yy instanceof EventAdapter.ViewHolder) {
                TextView tv = ((EventAdapter.ViewHolder) yy).title;
                if (tv != null) {
                    title = (String) tv.getText();
                }
            } else if (yy != null) {
                TextView dateView = ((EventByDayAdapter.ViewHolder) yy).dateView;
                if (dateView != null) {
    ...
    public void setSelectedView(View v) {
        if (v != null) {
            Object vh = v.getTag();
            if (vh instanceof EventAdapter.ViewHolder) {
                mSelectedVH = (EventAdapter.ViewHolder) vh;
    ...
    public EventAdapter.ViewHolder getSelectedViewHolder() {
        return mSelectedVH;
    }
