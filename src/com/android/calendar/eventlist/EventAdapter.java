...
//import android.text.format.DateFormat;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
import android.text.format.DateUtils;
import android.text.format.Time;
import android.view.View;
//import android.view.ViewGroup;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
import android.widget.LinearLayout;
import android.widget.ResourceCursorAdapter;
import android.widget.TextView;

//import com.android.calendar.ColorChipView;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
import android.widget.ImageView;//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
public class EventAdapter extends ResourceCursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (holder == null) {
            ...
            holder.img = (ImageView)view.findViewById(R.id.agenda_item_img);
            //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end

        }

        holder.startTimeMilli = cursor.getLong(EventWindowAdapter.INDEX_BEGIN);
        // Fade text if event was declined and set the color chip mode (response
        boolean allDay = cursor.getInt(EventWindowAdapter.INDEX_ALL_DAY) != 0;
        holder.allDay = allDay;
        int selfAttendeeStatus = cursor.getInt(EventWindowAdapter.INDEX_SELF_ATTENDEE_STATUS);
...
//        holder.colorChip.setLayoutParams(params);
        long startDay = cursor.getLong(EventWindowAdapter.INDEX_START_DAY);
        long endDay = cursor.getLong(EventWindowAdapter.INDEX_END_DAY);
        
        if (allDay) {
            holder.img.setBackgroundResource(R.drawable.event_normal);
        } else if(endDay > startDay){   //stride day event
            holder.img.setBackgroundResource(R.drawable.event_normal);
        } else {
            holder.img.setBackgroundResource(R.drawable.event_normal);
        }
        ...
        int canRespond = cursor.getInt(EventWindowAdapter.INDEX_CAN_ORGANIZER_RESPOND);
        if (canRespond == 0) {
            String owner = cursor.getString(EventWindowAdapter.INDEX_OWNER_ACCOUNT);
            String organizer = cursor.getString(EventWindowAdapter.INDEX_ORGANIZER);
        ...
        holder.instanceId = cursor.getLong(EventWindowAdapter.INDEX_INSTANCE_ID);
        ...
        // What
        String titleString = cursor.getString(EventWindowAdapter.INDEX_TITLE);
        ...
        // When
        long begin = cursor.getLong(EventWindowAdapter.INDEX_BEGIN);
        long end = cursor.getLong(EventWindowAdapter.INDEX_END);
        String eventTz = cursor.getString(EventWindowAdapter.INDEX_TIME_ZONE);
        int flags = 0;
        String whenString;
   ...
   /* Recurring event icon is removed
        String rrule = cursor.getString(EventWindowAdapter.INDEX_RRULE);
        ...
        */
        ...
        /*
        // Reminder
        boolean hasAlarm = cursor.getInt(EventWindowAdapter.INDEX_HAS_ALARM) != 0;
        if (hasAlarm) {
            updateReminder(view, context, begin, cursor.getLong(EventWindowAdapter.INDEX_EVENT_ID));
        }
        */

        // Where
        String whereString = cursor.getString(EventWindowAdapter.INDEX_EVENT_LOCATION);
