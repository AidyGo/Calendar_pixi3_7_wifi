...
/**
 * A helper class for deleting events. If a normal event is selected for deletion, then this pops up
 * a confirmation dialog. If the user confirms, then the normal event is deleted.
 * <p>
 * If a repeating event is selected for deletion, then this pops up dialog asking if the user wants
 * to delete just this one instance, or all the events in the series, or this event plus all
 * following events. The user may also cancel the delete.
 * </p>
 * <p>
 * To use this class, create an instance, passing in the parent activity and a boolean that
 * determines if the parent activity should exit if the event is deleted. Then to use the instance,
 * call one of the {@link delete()} methods on this class. An instance of this class may be created
 * once and reused (by calling {@link #delete()} multiple times).
 */
public class DeleteEventHelper {
    ...
    /**
     * Does the required processing for deleting an event, which includes first popping up a dialog
     * asking for confirmation (if the event is a normal event) or a dialog asking which events to
     * delete (if the event is a repeating event). The "which" parameter is used to check the
     * initial selection and is only used for repeating events. Set "which" to -1 to have nothing
     * selected initially.
     * 
     * @param begin the begin time of the event, in UTC milliseconds
     * @param end the end time of the event, in UTC milliseconds
     * @param eventId the event id
     * @param which one of the values {@link DELETE_SELECTED}, {@link DELETE_ALL_FOLLOWING},
     *            {@link DELETE_ALL}, or -1
     */
    ...
    /**
     * Does the required processing for deleting an event. This method takes a
     * {@link CalendarEventModel} object, which must have a valid uri for referencing the event in
     * the database and have the required fields listed below. The required fields for a normal
     * event are:
     * <ul>
     * <li>Events._ID</li>
     * <li>Events.TITLE</li>
     ...
     * </ul>
     * If the event no longer exists in the db this will still prompt the user but will return
     * without modifying the db after the query returns.
     * 
     * @param begin the begin time of the event, in UTC milliseconds
     * @param end the end time of the event, in UTC milliseconds
    ...
    public void delete(long begin, long end, CalendarEventModel model, int which) {
        ...
        if (TextUtils.isEmpty(rRule)) {
            AlertDialog dialog = new AlertDialog.Builder(mContext)
                    ...
                    .create();
            int positiveButton = DialogInterface.BUTTON_POSITIVE;
            int nagativeButton = DialogInterface.BUTTON_NEGATIVE;
            if (isExchangeDialog()) {
                positiveButton = DialogInterface.BUTTON_NEGATIVE;
                nagativeButton = DialogInterface.BUTTON_POSITIVE;
            }
            dialog.setButton(nagativeButton,
            ...
            if (originalEvent == null) {
                ...
                dialog.setButton(positiveButton,
     ...
     * @return
     */
    private boolean isExchangeDialog() {
        String language = Locale.getDefault().getLanguage();
        Log.i("aidy", "DeleteEventHelper -- isExchangeDialog() -- language = " + language);
