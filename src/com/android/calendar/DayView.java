...
import android.graphics.Color;//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
//import android.view.ContextMenu.ContextMenuInfo;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
//import android.widget.Toast;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
//import android.text.Layout.Alignment;//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    private static int DEFAULT_CELL_HEIGHT = 42;//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
    ...
    */
    ...
    private static float AMPM_TEXT_SIZE = 14;
    private static float AMPM_TOP_MARGIN = 12;
    ...
    private static int mAmPmColorRes;
    public static  int DEFAULT_AM_HEIGHT = 18;
    ...
    public DayView(Context context, CalendarController controller,
        ...
        if (mScale == 0) {
            ...
            if (mScale != 1) {
                ...
                //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
                DEFAULT_AM_HEIGHT *=mScale;
                AMPM_TOP_MARGIN *=mScale;
                //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
            }
        }
    ...
    private void init(Context context) {
        ...
        mAmPmColorRes = mResources.getColor(R.color.month_week_bgcolor);//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
        ...
        //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
        p.setTextSize(HOURS_TEXT_SIZE);
        mHoursWidth = Math.max(HOURS_MARGIN, computeMaxStringWidth(mHoursWidth, mHourStrs, p)
                + HOURS_RIGHT_MARGIN +HOURS_LEFT_MARGIN );
        //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
    ...
    public void handleOnResume() {
        ...
        //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
        mIs24HourFormat =  true/*DateFormat.is24HourFormat(mContext)*/;
        //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
    ...
    private void remeasure(int width, int height) {
        ...
        mGridAreaHeight = height - mFirstCell - DEFAULT_AM_HEIGHT * 2;//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
//                break;
    ...
    private class GotoBroadcaster implements Animation.AnimationListener {
        ...
        public void onAnimationStart(Animation animation) {
            //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
            DayView view = (DayView) mViewSwitcher.getCurrentView();
            view.mController.sendMessageHandler(CalendarController.MESSAGE_ID_GO_TIME_FOR_HEADER, mStart);
            //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
        }
    }
    ...
    //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
    public View switchViewsEX(boolean forward , Time goTime) {
        float inFromXValue, inToXValue;
        float outFromXValue, outToXValue;
...
//                Trace.traceCounter(Trace.TRACE_TAG_PERF, CALENDAR_APPUPDATE, 0);
    ...
    private void drawAllDayHighlights(Rect r, Canvas canvas, Paint p) {
        ...
        r.left = 0;
        r.right = mViewWidth;
        p.setColor(mFutureBgColor);//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
        p.setStyle(Style.FILL);
        canvas.drawRect(r, p);
    ...
    //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
    private void drawAmPmEx(Rect r,Canvas canvas){
        Paint p = new Paint();
        p.setColor(mAmPmColorRes);
        p.setAntiAlias(true);
        int amY = 0 ;
        //draw am
        r.top = amY;
        r.bottom = DEFAULT_AM_HEIGHT;
        r.left = 0;
        r.right = mViewWidth;
        canvas.drawRect(r, p);
        //draw pm
        final float deltaY = mCellHeight + HOUR_GAP;
        int pmY = (int) (DEFAULT_AM_HEIGHT + deltaY *13) ;
        r.top = pmY;
        r.bottom = r.top + DEFAULT_AM_HEIGHT;
        r.left = 0;
        r.right = mViewWidth;
        canvas.drawRect(r, p);
        ...
        p.setTypeface(mBold);
        float x0 = mHoursWidth - p.measureText(mAmString);
        canvas.drawText(mAmString, x0/2, amY + AMPM_TOP_MARGIN, p);
        float x1 = mHoursWidth - p.measureText(mAmString);
        canvas.drawText(mPmString, x1/2, pmY + AMPM_TOP_MARGIN, p);
        }
    //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
    ...
    private void doDraw(Canvas canvas) {
        ...
        drawAmPmEx(r,canvas);//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
        ...
        for (int day = 0; day < mNumDays; day++, cell++) {
            ...
            if (cell == mTodayJulianDay) {
               //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
               if(mCurrentTime.hour < 13){
                   lineY = mCurrentTime.hour * (mCellHeight + HOUR_GAP)
                         + ((mCurrentTime.minute * mCellHeight) / 60) + 1 + DEFAULT_AM_HEIGHT;
                }else{
                   lineY = mCurrentTime.hour * (mCellHeight + HOUR_GAP)
                              + ((mCurrentTime.minute * mCellHeight) / 60) + 1 + DEFAULT_AM_HEIGHT * 2;
                  }
               //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            int daynum = mSelectionDay - mFirstJulianDay;
            //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
            if(mSelectionHour < 13){
                r.top = mSelectionHour * (mCellHeight + HOUR_GAP) + DEFAULT_AM_HEIGHT ;
            }else{
                r.top = mSelectionHour * (mCellHeight + HOUR_GAP) + DEFAULT_AM_HEIGHT * 2;
             }
            //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
    ...
    private void drawHours(Rect r, Canvas canvas, Paint p) {
        ...
        int y = HOUR_GAP + mHoursTextHeight/2 + HOURS_TOP_MARGIN + DEFAULT_AM_HEIGHT + mCellHeight/2;
        for (int i = 0; i < 24; i++) {
           ...
           float x = mHoursWidth - p.measureText(time);
           canvas.drawText(time, x/2, y, p);
    ...
    private void setupHourTextPaint(Paint p) {
        ...
        //p.setTextAlign(Paint.Align.RIGHT);//deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
        p.setAntiAlias(true);
    }
    ...
    private void drawGridBackground(Rect r, Canvas canvas, Paint p) {
        ...
        final float stopY = HOUR_GAP + 24 * (mCellHeight + HOUR_GAP) + DEFAULT_AM_HEIGHT * 2;
        float x = mHoursWidth;
        ...
        for (int hour = 0; hour <= 24; hour++) {
           ...
           mLines[linesIndex++] = GRID_LINE_LEFT_MARGIN + mHoursWidth;
           mLines[linesIndex++] = y;
    ...
    private void drawBgColors(Rect r, Canvas canvas, Paint p) {
        ...
        if (mNumDays == 1 && todayIndex == 0) {
            ...
            //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
            if(mCurrentTime.hour < 13){
               lineY += DEFAULT_AM_HEIGHT;
            }else{
               lineY += DEFAULT_AM_HEIGHT * 2;
              }
            //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end

            if (lineY < mViewStartY + mViewHeight) {
                lineY = Math.max(lineY, mViewStartY);
                r.left = (int) GRID_LINE_LEFT_MARGIN;//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
                r.right = mViewWidth;
                r.top = lineY;
...
//            }
    ...
    private Rect drawEventRect(Event event, Canvas canvas, Paint p, Paint eventTextPaint,
       ...
       //FR593012-Wentao-Wan-001 begin for PR430744
       int width = (int) event.right - (int)event.left ;
       boolean isChange = false ;
		float minEventRectWidth = mResources.getDisplayMetrics().density * 3;
		if (width > minEventRectWidth) {
          isChange = true ;
        }
        //FR593012-Wentao-Wan-001 end for PR430744
        // Draw the Event Rect
        Rect r = mRect;
        ...
        int ceilHalfStroke = (int) Math.floor(EVENT_RECT_STROKE_WIDTH / 2.0f);
        //FR593012-Wentao-Wan-001 end for PR430744
        ...
        if (y < mFirstCell) {
            ...
            if (adjustedY < mFirstHourOffset) {
                ...
                setSelectedHour(mSelectionHour +
                        (adjustedY - mFirstHourOffset -DEFAULT_AM_HEIGHT * 2) / (mCellHeight + HOUR_GAP));
                //modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
            }
