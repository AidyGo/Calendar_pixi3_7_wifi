...
import android.graphics.Paint.FontMetrics;//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
...
public class MonthWeekEventsView extends SimpleWeekView implements IMonthViewForExt {
    ...
    private static int TEXT_SIZE_MONTH_NUMBER = 18;//FR593012-Wentao-Wan-001
    ...
    private static int TOP_PADDING_MONTH_NUMBER = 12;//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
    private static int TOP_PADDING_WEEK_NUMBER = 16;
    ...
    private static int EVENT_CRICLE_RADIUS = 2;//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
    ...
    protected void loadColors(Context context) {
        ...
        mMonthBGTodayColor = res.getColor(R.color.today_background_color);//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
    ...
    protected void initView() {
        ...
        if (!mInitialized) {
            ...
            if (mScale != 1) {
                ...
                EVENT_CRICLE_RADIUS *= mScale;//added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
            }
            if (!mShowDetailsInMonth) {
        ...
        mMonthNumPaint.setTextAlign(Align.CENTER);//FR593012-Wentao-Wan-001
    ...
    //Modified by guangqian.peng for PR707270 start.
    //FR593012-Wentao-Wan-001 begin
    private void drawClickEX(Canvas canvas) {
       /*//Delete by guangqian.peng for PR707270
        if(Utils.getSelectedWeekDayEX() != mUniqueID){
           return;
        }
        */

		int clickedDayIndex = Utils.getClickedDayIndexEX();
		if (clickedDayIndex == -1) {
			return;
		}
		
		int day = mFirstJulianDay + clickedDayIndex;
		Time time = new Time(mTimeZone);
		Utils.setJulianDayInGeneral(time, day);
		int weekNumber = Utils.getWeekNumberFromTime(time.toMillis(true),this.getContext());
		
		if (weekNumber != Utils.getSelectedWeekDayEX()) {
			return;
		}
  ...
  //FR593012-Wentao-Wan-001 end
    //Modified by guangqian.peng for PR707270 end.
    ...
    protected void drawWeekNums(Canvas canvas) {
        ...
        Utils.monthDayViewWidth = effectiveWidth / mNumDays;
        //FR593012-Wentao-Wan-001 end
        if (mShowWeekNum) {
            x = SIDE_PADDING_WEEK_NUMBER + mPadding;
            y = mWeekNumAscentHeight + TOP_PADDING_WEEK_NUMBER;//FR593012-Wentao-Wan-001
		...
		//yuanding modify it for PR:635523 20140506 start
        y = mWeekNumAscentHeight + TOP_PADDING_MONTH_NUMBER+10;//FR593012-Wentao-Wan-001
		//yuanding modify it for PR:635523 20140506 end
        ...
        for (; i < numCount; i++) {
            if (mHasToday && todayIndex == i) {
                mMonthNumPaint.setColor(Color.WHITE/*mMonthNumTodayColor*/);//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
			...
			//yuanding modify it for the day is offset for PR635523 20140411 start
            x = computeDayLeftPosition(i - offset) - (SIDE_PADDING_MONTH_NUMBER)-10;
			//yuanding modify it for the day is offset for PR635523 20140411 end
    ...
    //FR593012-Wentao-Wan-001 begin
    private int TRI_COLOR = Color.BLACK;
    private int Event_ICON_WIDTH = 10;
   //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
    protected void drawCricleEvents(Canvas canvas) {
        if (mEvents == null) {
            return;
        }
        Resources res = mContext.getResources();
        TRI_COLOR = res.getColor(R.color.event_triangle_color);
        ...
        for (ArrayList<Event> eventDay : mEvents) {
            ...
            String numStr = mDayNumbers[day];
            int numWidth = (int) mMonthNumPaint.measureText(numStr);
            float numHeight = (float) Math.ceil(fm.descent - fm.top) + 2;
            int radius = EVENT_CRICLE_RADIUS;
			//yuanding modify it for PR:635523 20140506 start
            float x = computeDayLeftPosition(day + 1) - (SIDE_PADDING_MONTH_NUMBER)  + numWidth/2-15  ;
            float y = TOP_PADDING_MONTH_NUMBER + mWeekNumAscentHeight - numHeight/2+5;
			//yuanding modify it for PR:635523 20140506 end

            mEventSquarePaint.setStyle(Style.FILL_AND_STROKE);
            mEventSquarePaint.setColor(TRI_COLOR);
            canvas.drawCircle(x + radius , y - radius * 2, radius, mEventSquarePaint);
        }
    }
   //added for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 end
    protected void drawTriAngleEvents(Canvas canvas) {
        if (mEvents == null) {
            return;
        }
        Resources res = mContext.getResources();
        ...
        for (ArrayList<Event> eventDay : mEvents) {
            ...
            int rightEdge = computeDayLeftPosition(day + 1) - 3;

            mEventSquarePaint.setStyle(Style.FILL_AND_STROKE);
            mEventSquarePaint.setColor(TRI_COLOR);
            Path path = new Path();
            path.moveTo(rightEdge - Event_ICON_WIDTH, mHeight);
            path.lineTo(rightEdge, mHeight - Event_ICON_WIDTH);
            path.lineTo(rightEdge, mHeight);
            path.close();
            canvas.drawPath(path, mEventSquarePaint);
        }
    }
    //FR593012-Wentao-Wan-001 end
