...
//import com.android.calendar.AllInOneActivity;deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
...
//import android.text.format.DateUtils;deleted for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13 begin
...
public class MonthByWeekAdapter extends SimpleWeeksAdapter {
    ...
    protected boolean mIsMiniMonth = false;//FR593012-Wentao-Wan-001
    ...
    private GestureDetector mGestureDetector;//FR593012-Wentao-Wan-001
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        setRealSelectedWeek();//FR593012-Wentao-Wan-001
    ...
    protected void onDayTapped(Time day) {
        ...
        */
        setSelectedDay(day);
        mController.sendMessageHandler(CalendarController.MESSAGE_ID_SET_TIME, day);
        mController.setTime(day.normalize(true));
        //FR593012-Wentao-Wan-001 end
         if (mShowAgendaWithMonth || mIsMiniMonth) {
            ...
            if (!mIsMiniMonth) {
              //FR593012-Wentao-Wan-001 begin
                ...
                */
              //FR593012-Wentao-Wan-001 end
                //go DayView,not go to previous view
                mController.setTime(day.normalize(true));
                mController.sendEvent(this, EventType.GO_TO, null, null, -1, ViewType.WEEK, CalendarController.EXTRA_GOTO_DATE//FR602091 porting from wave3 by yubin.yi.hz at 2014-02-14
                        | CalendarController.EXTRA_GOTO_BACK_TO_PREVIOUS, null, null);//FR593012-Wentao-Wan-001
            } else {
                mController.sendEvent(mContext, EventType.GO_TO, day, day, -1, mIsMiniMonth ? ViewType.CURRENT
    ...
    */
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        mSingleTapUpView = (MonthWeekEventsView) v;//FR593012-Wentao-Wan-001
        int action = event.getAction();
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mClickedView = (MonthWeekEventsView)v;
    ...
    protected class CalendarGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapUp(MotionEvent e) {
           ...
           {    mSelectedDay.set(day);
                int julianDay = Time.getJulianDay(mSelectedDay.toMillis(true), mSelectedDay.gmtoff);
                long millis = mSelectedDay.normalize(true);
                mSelectedWeek = Utils.getWeeksSinceEpochFromJulianDay(
                        Time.getJulianDay(millis, mSelectedDay.gmtoff), mFirstDayOfWeek);
                Time currentTime = new Time(mSelectedDay.timezone);
                currentTime.set(System.currentTimeMillis());
                boolean isSameYear = mSelectedDay.year == currentTime.year;
                boolean isSameMonth = mSelectedDay.month == currentTime.month;
                boolean isSameDay = mSelectedDay.monthDay == currentTime.monthDay;
                if(isSameYear && isSameMonth && isSameDay ){
                    Utils.clearBufferEX();
                }else{
                    int clickedDayIndex =mSingleTapUpView.getDayIndexFromLocation(mClickedXLocation);
					// Add by guangqian.peng for PR707270 start.
					int clickWeekNumIndex = Utils.getWeekNumberFromTime(day.toMillis(true), mContext);
					Utils.setSelectedWeekDayEX(clickWeekNumIndex);
					// Add by guangqian.peng for PR707270 end.
                    Utils.setClickedDayIndexEX(clickedDayIndex);
//                    Utils.setSelectedWeekDayEX(mSelectedWeek);//Delete by guangqian.peng for PR707270
                    Utils.setSelectedJulianDayEX(julianDay);
                  }
                notifyDataSetChanged();
                mController.sendMessageHandler(CalendarController.MESSAGE_ID_SET_TIME, day);//set SimpleDayPickerFragment mSelectedDay = day
                mController.setTime(day.normalize(true));
                mController.sendMessageHandler(CalendarController.MESSAGE_ID_GOTO_TIME, day);
            }
            //FR593012-Wentao-Wan-001 end
            return true;
        }
