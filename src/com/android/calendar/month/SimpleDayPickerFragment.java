...
import libcore.icu.LocaleData;//added by yuandinghz for CR596021 Calendar Ergo at 2014-02-28
...
public class SimpleDayPickerFragment extends ListFragment implements OnScrollListener {
    ...
    public static int LIST_TOP_OFFSET = 0;   // so that the bottom line will be under the shadow
   ...
   //added yuanding add it for PR635523 20140409 start
    private static long mTwoMonthLength = 5000000000l;
    protected ImageButton mGoToLastMonth;
    protected ImageButton mGoToNextMonth;
	//added yuanding add it for PR635523 20140409 end
    ...
    protected DataSetObserver mObserver = new DataSetObserver() {
        ...
        public void onChanged() {
            ...
            if (day.year != mSelectedDay.year || day.yearDay != mSelectedDay.yearDay) {
                goTo(day.toMillis(true), true, true, false,true);//FR593012-Wentao-Wan-001
            }
        }
    };
    ...
    public SimpleDayPickerFragment(long initialTime) {
        //FR593012-Wentao-Wan-001 begin
        goTo(initialTime, false, true, true,false);
    ...
    public void onAttach(Activity activity) {
        ...
        mContext = activity;
        String tz = Utils.getTimeZone(mContext, null);//modified for FR602091 Porting Wave3 Calendar to soul45 by yubin.yi.hz at 2014-02-13
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null && savedInstanceState.containsKey(KEY_CURRENT_TIME)) {
            goTo(savedInstanceState.getLong(KEY_CURRENT_TIME), false, true, true,false);//FR593012-Wentao-Wan-001
        }
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
		...
		//added yuanding add it for PR635523 20140409 start
	    mGoToLastMonth = (ImageButton)getView().findViewById(R.id.goto_last_month);
        mGoToNextMonth = (ImageButton)getView().findViewById(R.id.goto_next_month);
        
        mGoToLastMonth.setOnClickListener(new OnClickListener(){
        	public void onClick(View v) {
        		goTo(mTempTime.toMillis(true) - mTwoMonthLength, false, true, true,false);
        	}
        });
        
        mGoToNextMonth.setOnClickListener(new OnClickListener(){
        	public void onClick(View v) {
        		goTo(mTempTime.toMillis(true), false, true, true,false);
        	}
        });
    ...
    protected void doResumeUpdates() {
        ...
        updateHeader();
        goTo(mSelectedDay.toMillis(true), false, false, false,true);//FR593012-Wentao-Wan-001
        ...
        for (int i = 1; i < 8; i++) {
            ...
            if (i < mDaysPerWeek + 1) {
                ...
                if (position == Time.SATURDAY) {
                    label.setTextColor(mDayNameColor);//FR593012-Wentao-Wan-001
     ...
     */
    //FR593012-Wentao-Wan-001 begin
    public boolean goTo(long time, boolean animate, boolean setSelected, boolean forceScroll,boolean beNotUpdateMonthHighlight) {
        if (time == -1) {
        ...
        if (position < firstPosition || position > lastPosition || forceScroll) {
            ...
            */
            ...
            else
            {
                ...
                mFirstDayOfMonth.month = mSelectedDay.month;
                mFirstDayOfMonth.monthDay = mSelectedDay.monthDay;
                setMonthDisplayed(mFirstDayOfMonth, true);

                Time mGoToDayOfMonth = new Time();
                mGoToDayOfMonth.set(mSelectedDay);
                mGoToDayOfMonth.monthDay = 1;
                millis = mGoToDayOfMonth.normalize(true);
                position = Utils.getWeeksSinceEpochFromJulianDay(
                        Time.getJulianDay(millis, mGoToDayOfMonth.gmtoff), mFirstDayOfWeek);
                ...
                mListView.setOnScrollListener(new OnScrollListener(){
                    ...
                    public void onScrollStateChanged(AbsListView view,
                        ...
                        //goto MonthByWeekFragment.onScrollStateChanged ->mScrollStateChangedRunnable.doScrollStateChange(view,scrollState);
                        Message message = new Message();
                        message.what = scrollState;
                        message.obj = view;
    ...
    private void updateMonthHighlight(AbsListView view) {
        ...
        if (monthDiff != 0) {
            ...
            CalendarController.getInstance(mContext).setTime(mTempTime.normalize(true));//FR593012-Wentao-Wan-001
        }
    }
...
//        mMonthName.setText(Utils.formatMonthYear(mContext, time));
      //add and remove by xiaoyu.qian for pr:831159 begin
        //String[] sLongMonths = LocaleData.get(Locale.getDefault()).longMonthNames;
        String[] sLongMonths = LocaleData.get(Locale.getDefault()).longStandAloneMonthNames;
      //add and remove by xiaoyu.qian for pr:831159 begin
        mMonthName.setText(sLongMonths[time.month]);
    ...
    protected class ScrollStateRunnable implements Runnable {
        ...
        public void run() {
            ...
            {
                mListView.setOnScrollListener(SimpleDayPickerFragment.this);
            }
            //FR593012-Wentao-Wan-001 end
            ...
            if (mNewState == OnScrollListener.SCROLL_STATE_IDLE
                ...
                if (dist < LIST_TOP_OFFSET) {
                    ...
                    boolean scroll = firstPosition != 0 && lastPosition != mListView.getCount() - 1;
                    if (mIsScrollingUp && scroll) {
                    	mListView.smoothScrollBy(dist, 500);
                    } else if (!mIsScrollingUp && scroll) {
                    	mListView.smoothScrollBy(child.getHeight() + dist, 500);
                    }
                }
				//added yuanding add it for PR635523 20140409 end
                mAdapter.updateFocusMonth(mCurrentMonthDisplayed);
